...
class freetype_text_font_mask_draw {
private:
    FT_Library library_;
    FT_Face face_;
    ...
    std::mutex mutex_;
public:
    static freetype_text_font_mask_draw& get_instance() {
        static freetype_text_font_mask_draw instance;
        return instance;
    }
    ...
    int draw(unsigned char* font_mask, 
             ...
             int height, 
             int font_size) {
        std::unique_lock<std::mutex> lock(mutex_);      // can be called by mutiprocess
        mask_ = font_mask;
        text_ = str;
        width_ = width;
        height_ = height;
...
private:
    freetype_text_font_mask_draw() = default;
    virtual ~freetype_text_font_mask_draw() {
        uninit();
    }
    void uninit() {
        FT_Done_Face(face_);
...
#define FREETYPE_TEXT_FONT_MASK_DRAW freetype_text_font_mask_draw::get_instance()